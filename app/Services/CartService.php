...
class CartService extends Service
{
    ...
    {
        if(!$order->item){
            $this->response->addError($text = 'Пропущен неизвестный товар с артикулом. Пожалуйста сверьтесь с удаленной корзиной.');
            $this->toCartLog($order, $text);
            return false;
        }
        return true;
    }
    ...
    protected function isQtyMoreThanMin(Order $order, int $qty): bool
    {
        if($qty < $order->item->min_qty){
            $this->response->addError($text = 'Товар с артикулом '.$order->item->sid.' не добавлен в корзину, т.к. количество меньше минимального');
            $this->toCartLog($order, $text);
            return false;
        }
        return true;
    ...
    protected function checkResult(Order $order, $result): bool
    {
        if (is_object($result) && !empty($result->id)) {
            $this->response->addSuccess($text = 'Товар с артикулом ' . $order->item->sid . ' в количестве ' . $result->qty . ' ' . $order->item->pluralNameFormat . ' успешно добавлен');
            $this->toCartLog($order, $text);
            return true;
        } else {
            $this->response->addError($text = 'Товар с артикулом ' . $order->item->sid . ' не добавлен в корзину. Пожалуйста добавьте в ручную.');
            $this->toCartLog($order, $text);
            return false;
        }
    }
    ...
    {
        if ($qty != $result->qty) {
            $this->response->addError($text = 'Количество выбранного товара с артикулом ' . $order->item->sid . ' отличается от количества добавленного в корзину. Пожалуйста проверьте.');
            $this->toCartLog($order, $text);
        }
    }

    protected function toCartLog(Order $order, string $text): void
    {
        $order->cartLog()->updateOrCreate(['order_id' => $order->id], ['text' => $text]);
    }

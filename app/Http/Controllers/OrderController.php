...
class OrderController extends Controller
{
    ...
    public function storeExists(OrderStoreExistsRequest $request)
    {
        ...
        $delivery = $this->deliveryRepository->getDeliveryPrice($order->item->sid, $request->qty);
    ...
    public function storeNew(OrderService $orderService, OrderStoreNewRequest $request)
    {
        ...
        $delivery = $this->deliveryRepository->getDeliveryPrice($item->sid, $request->qty);
    ...
    public function edit(int $id, int $page, int $anchor)
    {
        return view('order.edit', compact('id', 'page', 'anchor'));
    }
    ...
    public function update(OrderUpdateRequest $request)
    {
        ...
        if($pivot->update($this->order->getOrderUserData($request->qty, $delivery))){
            ...
            return redirect($this->getRedirectUrl($request))->withSuccess([__('messages.updated', ['name'=>__('messages.order')])]);
            return back()->withSuccess([__('messages.updated', ['name'=>__('messages.order')])]);
        }
        return redirect($this->getRedirectUrl($request))->withErrors([__('messages.error.updated')]);
    ...
    protected function getRedirectUrl(Request $request): string {
        return '/'. (!empty($request->page) ? '?page='.$request->page : '') . $request->anchor;
    }
}
